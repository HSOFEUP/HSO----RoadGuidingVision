#include "MSAC.h"
#include "errorHSO.h"
#include "../LMLSM/lmfit-3.2/lib/lmmin.h"
...
void MSAC::fillDataContainers(std::vector<std::vector<cv::Point> > &lineSegments)
{
	...
	for (int i=0; i<numLines; i++)
	{
		...
		else // __mode == MODE_HSO requires not to calibrate into the sphere
		{
			__an = __a;
			__bn = __b;
		}
...
{	
	...
	for(int vpNum=0; vpNum < numVps; vpNum++)
	{
		...
		{		
			...
			else if(__mode == MODE_HSO)
				estimateHSO(__Li, __Lengths, __Mi, ind_CS, __N_I_best, __vp);	// Output __vp is calibrated
			else
				perror("ERROR: mode not supported, please use {LS, LIEB, HSO}\n");
...
{	
	...
	if(__mode == MODE_LS)
		...
		estimateHSO(Li, Mi, Lengths, MSS, 2, vp);
	else
		perror("ERROR: mode not supported. Please use {LS, LIEB, HSO}\n");
}
...
float MSAC::GetConsensusSet(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp, std::vector<float> &E, int *CS_counter)
{
	...
	if(__mode == MODE_LS)
		...
		J = errorHSO(vpNum, Li, Lengths, Mi, vp, E, CS_counter);
	else
		perror("ERROR: mode not supported, please use {LS, LIEB, HSO}\n");
...
void MSAC::estimateHSO(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &set, int set_length, cv::Mat &vp)
{
	if (set_length == __minimal_sample_set_dimension)
	{	
		// Just the cross product
		// DATA IS NOT CALIBRATED for MODE_HSO
		cv::Mat ls0 = Mat(3,1,CV_32F);
		cv::Mat ls1 = Mat(3,1,CV_32F);
	...
	for(int t=0; t<numTheta; t++)
	{
		...
		{		
			...
			evaluateHSO(paramTest, set_length, (const void*)&dataTest, fvecTest, infoTest);
	...
	lmmin(num_par, par, m_dat, &data, evaluateHSO, &control, &status, lm_printout_std);
...
float MSAC::errorHSO(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp, std::vector<float> &E, int *CS_counter)
{
	float J = 0;
	float di = 0;
	...
	{ 		
		...
		di = distanceHSO(vn, lineSegment, lengthLineSegment, midPoint);
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	for(unsigned int vpNum=0; vpNum<vps.size(); vpNum++)
	{
		if(vps[vpNum].at<float>(2,0) != 0)
		{
			...
			{
				circle(im, vp, 4, colors[vpNum+1], 2);	
