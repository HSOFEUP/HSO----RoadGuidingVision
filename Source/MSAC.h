...
private:
	// Error Mode
	int __mode;	// Error mode (MODE_LS or MODE_HSO)
...
private:	
	...
	/** This function estimates the vanishing point for a given set of line segments using the HSO's method*/
	void estimateHSO(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &set, int set_length, cv::Mat &vEst);
	...
	/** This function computes the residuals of the line segments given a vanishing point using*/
	float errorHSO(int vpNum, cv::Mat &Li, cv::Mat &lengthsLS, cv::Mat &Mi, cv::Mat &vp, std::vector<float> &E, int *CS_counter);
