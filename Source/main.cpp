...
void help()
{
	 cout << "/*\n"
		 ...
		 << " *		-mode		# Estimation mode (default is HSO): LS (Least Squares), HSO\n"
...
#ifndef USE_PPHT
	vector<Vec2f> lines;
	cv::HoughLines( imgCanny, lines, 1, CV_PI/180, 10);
...
{	
	...
	for(int i=1; i<argc; i++)
	{
		...
		else if(strcmp(s, "-mode" ) == 0)
		{
			...
			if(strcmp(ss, "LS") == 0)
				mode = MODE_LS;
			else if(strcmp(ss, "HSO") == 0)
				mode = MODE_HSO;
			else
			{
				perror("ERROR: Only LS or HSO modes are supported\n");
			}
		}
